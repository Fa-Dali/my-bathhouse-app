...
export default function Page({ }: PageProps) {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        ...
        if (!token) {
          setUserRole('other'); // аноним — только просмотр
          return;
        }
        ...
        const roles = response.data.roles.map((r: any) => r.code);
        setUserRole(roles.includes('admin') ? 'admin' : 'other');
      } catch (error) {
        console.error('Ошибка загрузки профиля:', error);
        setUserRole('other'); // ошибка → только просмотр
      }
    };
  ...
  return (
    ...
    >
      {userRole === null ? (
        <div className="flex justify-center p-8">Проверка прав...</div>
      ) : (
        <div> {/* ← Сюда, внутрь этого <div> */}
          {/* ✅ СЮДА ВСТАВЛЯЕМ ПОДСКАЗКУ */}
          {isReadOnly && (
            <div className="flex items-center justify-center gap-2 text-sm text-red-600 bg-red-100 py-2 mb-3 rounded border">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
              </svg>
              <p className="text-red-600 text-lg">Только администратор может редактировать отчёт</p>
            </div>
          )}
                ...
                <div className=" beautiful-scroll overflow-y-auto max-h-[300px]">
                  ...
                  <div className="sm:hidden fixed bottom-0 left-0 right-0 bg-gray-100 border-t shadow-lg z-50">
                    <div className="flex justify-around items-center p-1 text-xs">
                      ...
                      <button
                        onClick={clearTable}
                        className="m-1 border rounded-md border-slate-500 p-2 text-white bg-red-500"
                        disabled={isReadOnly}
                      >
